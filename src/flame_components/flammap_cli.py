...
def _get_supporting_data_path() -> str:
    """Return the configured writable directory for vendor supporting data."""
    configured_path = os.environ.get("FLAME_COMPONENTS_DATA_DIR")
    if configured_path:
        return os.path.abspath(configured_path)

    repository_path = os.path.join(project_path, "supporting_data")
    if os.path.isdir(os.path.join(project_path, ".git")):
        return repository_path

    app_data_path = os.environ.get("LOCALAPPDATA", os.path.expanduser("~"))
    return os.path.join(app_data_path, "flame_components", "supporting_data")


supporting_data_path = _get_supporting_data_path()
fb_path = os.path.join(supporting_data_path, "FB")
...
def download_apps() -> None:
    ...
    os.makedirs(supporting_data_path, exist_ok=True)
    ...
    if response.status_code == 200:
        ...
        zip_file_path = os.path.join(supporting_data_path, "FireBehaviorModels.zip")
...
def run_app(
    ...
    """
    # Check if the FB folder exists within the supporting_data folder
